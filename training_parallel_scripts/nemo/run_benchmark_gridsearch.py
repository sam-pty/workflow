...
# Output CSV file
OUTPUT_CSV = f"benchmark_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def run_experiment(tp_size, pp_size, cp_size, csv_writer):
    ...
    if dp_size < 1 or not dp_size.is_integer():
        ...
        csv_writer.writerow({
            'TP': tp_size, 'PP': pp_size, 'CP': cp_size, 'DP': 'INVALID',
    ...
    try:
        ...
        if len(step_times) > 5:
            ...
            csv_writer.writerow({
                'TP': tp_size, 'PP': pp_size, 'CP': cp_size, 'DP': int(dp_size),
                'Status': status, 'Error': '',
...
def run_grid_search():
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        fieldnames = ['TP', 'PP', 'CP', 'DP', 'Status', 'Error', 
