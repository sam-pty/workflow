...
class Mesh final : public BasicTopology {
  public:
     ...
     * Constructor
     *
     * @param npus_count number of NPUs in the Mesh
     * @param bandwidth bandwidth of each link
     * @param latency latency of each link
     * @param bidirectional whether the mesh is bidirectional, defaults to true
     */
    Mesh(int npus_count, Bandwidth bandwidth, Latency latency, bool bidirectional = true) noexcept;
  ...
  private:
    ...
    /// true if the mesh is bidirectional, false otherwise
    bool bidirectional;
};
